...
public class SuitInterface extends MenuFactory {

    public SuitInterface(Player player, Island island, Minion minion){
        super("Available Suits for your island.", 4);
        ...
        if (island.getAvailableSuits().size() < 1) {
            addItem(new MenuItem(13, new ItemFactory(Material.PAPER).setDisplayName("&c&lYou have no available suits!").build()));
        ...
        for (SuitType available : island.getAvailableSuits()) {
            addItem(new MenuItem(slot, new ItemFactory(Material.PAPER)
            .setDisplayName("&8[&b" + StringUtil.capitalizeFirstLetter(available.name()) + "&8] &bSuit")
                    .setLore(
            ...
            .build()) {
                ...
                public void click(Player player, ClickType clickType) {
                    if (minion != null) {
                        minion.setSuit(available.newSuit(minion));
