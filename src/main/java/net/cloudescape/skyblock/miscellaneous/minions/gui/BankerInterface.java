 ...
 */
public class BankerInterface extends MenuFactory{


    public BankerInterface(Player player, Minion minion){
        super(minion.getType().name() + "'s Interface", 4);

        if (!(minion instanceof Banker)) {
            return;
        }
...
//                                "&8• &7Minion Tag - &c" + (minion.getMinionTag() == null || minion.getMinionTag().getTag() == null ? "None" : minion.getMinionTag().getTag().replace("\\[|\\]", ""))
                        ).build()));
        ...
        addItem(new MenuItem(11, new ItemFactory(Material.EXP_BOTTLE)
            ...
            public void click(Player player, ClickType clickType) {
                ...
                if (clickType.isRightClick()) {
                    ...
                    minion.setHunger(banker.getHunger() + 2);
        ...
        addItem(new MenuItem(13, new ItemFactory(Material.PAPER)
                .setDisplayName("&aInvestements")
                .setLore(
                        StringUtil.getMenuLine(),
                        " &7You can invest at " + (StringUtil.getDurationBreakdown(banker.getCanWithdraw())),
                        " &7This minion has produced &b$" + banker.getInvestment() + " &7!",
                        " &7Left click - &cGet your investment",
                        " &7Right click - &cCancel Investing",
                        StringUtil.getMenuLine()
                )
                .build()) {
            ...
            public void click(Player player, ClickType clickType) {
                ...
                if (clickType == ClickType.LEFT) {
                    if (!banker.canWithdraw()) {
                        CustomChatMessage.sendMessage(player, "Banker", "You cannot withdraw your investment yet.");
                        return;
                    }

                    // TODO
                    banker.getIsland().setBalance((int) (banker.getIsland().getBalance() + banker.getInvestment()));
                } else {
                    banker.finishInvestment();
                }
            }
        });
